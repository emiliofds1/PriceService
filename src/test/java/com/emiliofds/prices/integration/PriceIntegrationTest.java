...
class PriceIntegrationTest {
  ...
  void getPrice_givenNoProductID_shouldReturnsBadRequest() throws Exception {
    performPriceRequest(null, BRAND_ID, VALID_DATE).andExpect(status().isBadRequest());
  }

  @Test
  void getPrice_givenNoBrandID_shouldReturnsBadRequest() throws Exception {
    performPriceRequest(PRODUCT_ID, null, VALID_DATE).andExpect(status().isBadRequest());
  }

  @Test
  void getPrice_givenNoDate_shouldReturnsBadRequest() throws Exception {
    performPriceRequest(PRODUCT_ID, BRAND_ID, null).andExpect(status().isBadRequest());
  }

  @Test
  void getPrice_givenInvalidParameters_shouldReturnsNotFound() throws Exception {
    performPriceRequest(INVALID_PRODUCT_ID, BRAND_ID, VALID_DATE).andExpect(status().isNotFound());
  }

  @Test
  void whenDateOutOfRange_thenReturnsNotFound() throws Exception {
    performPriceRequest(PRODUCT_ID, BRAND_ID, OUT_OF_RANGE_DATE).andExpect(status().isNotFound());
  }

  @Test
  void whenPriceValueIsInvalid_thenReturnsBadRequest() throws Exception {
    performPriceRequest("2", "1", VALID_DATE).andExpect(status().isInternalServerError());
  }
  ...
  private ResultActions performPriceRequest(String productId, String brandId, String date)
      throws Exception {
  ...
  private void addParamIfNotNull(
      MockHttpServletRequestBuilder request, String paramName, String paramValue) {
    if (paramValue != null) {
